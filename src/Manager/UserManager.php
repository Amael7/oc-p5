...
class UserManager extends AppManager {
   ...
   * @return User
   */
  public static function getUserByEmailAndPassword($email, $password, $className) {
    return parent::dbConnect()->query('SELECT * FROM ' . parent::getTableName($className) . ' WHERE email = ' . "'$email'" . ' AND password = ' . $password, parent::getClassName($className));
  }
}